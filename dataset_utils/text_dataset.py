...
def get_dataset(dataset_name, metadata=False, synthetic_train_path=None):
    ...
    elif 'seq2seq' in dataset_name:
        dataset_path = 'datasets/' + dataset_name
        dataset = load_dataset("text", data_files={f'{split}': os.path.join(dataset_path, f'{split}.jsonl') for split in ['train', 'valid', 'test']})
        dataset = process_seq2seq_dataset(dataset)
...
def process_seq2seq_dataset(dataset):
    def process_seq2seq_text(example):
        dict_example = json.loads(example['text'])
        dict_example['text'] = dict_example['output']
        dict_example['context'] = dict_example['input']
        key_list = list(dict_example.keys())
        for k in key_list:
            if k != 'text' and k != 'context':
                del dict_example[k]
        return dict_example
    dataset = dataset.map(process_seq2seq_text, )
    dataset = dataset.shuffle(seed=42)
    return dataset   
...
def get_dataloader(args, dataset, model_config, tokenizer, max_seq_len, mode='diffusion', shuffle=True, context_tokenizer=None):
    def tokenization(example):
        ...
        if mode == 'diffusion' and (args.dataset_name in {'xsum', 'qqp', 'delibot', 'wmt14-en-de', 'wmt14-de-en'} or 'utt' in args.dataset_name or ('seq2seq' in args.dataset_name)):
            # import pdb; pdb.set_trace()
            assert context_tokenizer is not None
            source = example['context']
            target = example['text']

            if args.dataset_name in {'qqp', 'delibot', 'wmt14-en-de', 'wmt14-de-en', 'roc_utt_small'} or 'utt' in args.dataset_name or ('seq2seq' in args.dataset_name):
    ...
    if args.dataset_name in {'xsum', 'qqp', 'delibot'} or ('wmt14' in args.dataset_name or 'utt' in args.dataset_name or ('seq2seq' in args.dataset_name)):
